...
const AppProvider = ({ children }) => {
	...
	// Fetch userInformation on start to see if userIs loggedIn
	useEffect(() => {
		(async function () {
			let { data } = await axios.get("http://localhost/unifin/ajax/payment.php");
			console.log(data);

			if (data === "User is not logged in") {
				// window.location = "../login";
			}
			if (data === "User is already a member") {
				// window.location = "../";
			}
		})();
	}, []);
	...
	async function postData(data) {
		...
		let result = await axios.post("http://localhost:90/unifin/ajax/payment.php", {
			data: data,
		});
